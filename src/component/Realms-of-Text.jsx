...
const ITEM_IMAGES = {
    // --- CONSUMABLES ---
    'Health Potion': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/thw9z9v2t152r4tj4wnreuh6osu4',
    'Mana Potion': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/2sqiv86xt5oacljfhmemaobw0cku',
    'Elixir of Strength': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/ey8s9po4b70n2rdsmhmn7m05gaoj',
    'Stoneskin Brew': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/6cjxfzdaip6f6d25uhgosfq2amve',

    // --- STARTER GEAR ---
    'Rusty Dagger': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/uag4rggwnvnxq9p2dumx9pfdkkli',
    'Cloth Shirt': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/w2z92jlud89afmlfbrsrbykapbn8',

    // --- COMMON SHOP GEAR ---
    'Iron Sword': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/u3h8k1xlzkz0ieirglbtropfw67w',
...
const ENEMY_IMAGES = {
  // --- STANDARD ENEMIES ---
  'Slime': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/fy6nnl9702h2u5tktbagmgl264o9',
  'Goblin': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/bnp2ixpul1aii4njxu9jmn4uwwmr',
  'Wolf': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/88lm3ss5glunh2zub6pwqhttuoq0',
  'Skeleton': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/es3i6cs6ad1gsx2l3bhcsajxclaa',
  'Orc': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/vs5pv7ohhqztk5ux0p4omig0im3s',
  'Dragon Whelp': 'https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/2vrxjapk1xv3608sa9y4nqgdw7ow',
...
//       else if (n.includes('ear') || n.includes('feather')) { emoji = '👂'; bg = '#14532d'; }
//       else if (n.includes('pelt') || n.includes('fur') || n.includes('hide')) { emoji = '🧶'; bg = '#78350f'; }
//       else if (n.includes('dust') || n.includes('bone')) { emoji = '🦴'; bg = '#57534e'; }
//       else if (n.includes('tusk') || n.includes('tooth')) { emoji = '🦷'; bg = '#713f12'; }
...
//   };
  ...
  // --- Enchanting System ---
    ...
    const getItemImage = (name, rarity = 'common') => {
        // 1. Check if we have a real image for this item
        if (ITEM_IMAGES[name]) {
              ...
              <div className="max-w-5xl w-full">
                  ...
                  <div className="grid md:grid-cols-3 gap-6">
                      {Object.values(CLASSES).map(cls => (
                        //   <button key={cls.id} onClick={() => selectClass(cls.id)} className="bg-slate-800 p-6 rounded-xl border-2 border-slate-700 hover:border-yellow-500 transition-all flex flex-col items-center text-center group relative overflow-hidden">
                        ...
                        //           <div className="flex justify-between"><span>HP Bonus:</span> <span className={cls.hpBonus>=0?'text-green-400':'text-red-400'}>{cls.hpBonus}</span></div>
                        //           <div className="flex justify-between"><span>Mana Bonus:</span> <span className="text-blue-400">{cls.manaBonus}</span></div>
                        //       </div>
                        //   </button>
                          <button
                              key={cls.id}
  ...
  return (
    <div className={`min-h-screen bg-slate-900 text-slate-200 font-mono flex flex-col md:flex-row overflow-hidden ${shake ? 'animate-shake' : ''}`}>
        ...
        <div className="bg-slate-700 p-3 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] md:text-xs">
           ...
           {/* <div className="mt-2 pt-2 border-t border-slate-600 flex gap-2">
               <div className={`w-8 h-8 rounded border ${player.equipment.weapon.rarity === 'epic' ? 'border-purple-500 shadow-[0_0_10px_#a855f7]' : 'border-slate-500'}`}>
                   <img src={getItemImage(player.equipment.weapon.name, player.equipment.weapon.rarity)} className="w-full h-full rounded" />
               </div>
               <div className={`w-8 h-8 rounded border ${player.equipment.armor.rarity === 'epic' ? 'border-purple-500 shadow-[0_0_10px_#a855f7]' : 'border-slate-500'}`}>
           ...
           {/* Equipment Display */}
                  <div className="mt-2 pt-2 border-t border-slate-600 flex gap-2">
                      ...
                      {/* WEAPON SLOT */}
                      <div className={`w-20 h-auto rounded border transition-all duration-500
      ...
      <div className="flex-1 flex flex-col h-[100vh] relative">
        ...
        {/* Scene Visuals */}
        <div className="h-40 bg-slate-950 border-b border-slate-700 p-4 flex items-center justify-center relative overflow-hidden shrink-0">
           ...
           <div className="z-10 text-center w-full max-w-xl">
              {/* {gameState === 'COMBAT' && enemy && (
                <div className="animate-in fade-in zoom-in duration-300">
                  <div className={`text-5xl mb-2 ${enemy.isElite ? 'animate-pulse' : ''}`}>👾</div>
                  <div className={`text-lg font-bold ${enemy.isElite ? 'text-red-500' : 'text-red-300'}`}>{enemy.name} <span className="text-xs bg-slate-800 px-1 rounded">Lvl {enemy.level}</span></div>
                  <div className="w-full bg-slate-800 h-2 mt-1 rounded-full overflow-hidden border border-slate-700">
                    <div className="bg-red-600 h-full transition-all duration-300" style={{width: `${(enemy.hp / enemy.maxHp) * 100}%`}}></div>
                  </div>
                </div>
              )} */}
                      {gameState === 'COMBAT' && enemy && (
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-900 custom-scrollbar">
          {logs.map((log) => (
            <div key={log.id} className={`p-2 rounded text-xs md:text-sm border-l-2 animate-fade-in ${
              log.type === 'danger' ? 'bg-red-950/30 border-red-500 text-red-200' :
        ...
        {/* Controls Area */}
        <div className="bg-slate-800 p-3 border-t border-slate-700 min-h-[150px] flex flex-col justify-center shrink-0 z-10">
            ...
            <div className="flex flex-col h-full">
                <div className="flex gap-2 mb-2 overflow-x-auto pb-2">
                    {SHOP_ITEMS.map(item => {
                        ...
                        return (
                            <button key={item.id} onClick={() => handleBuyItem(item)} className="bg-slate-700 hover:bg-slate-600 p-2 rounded text-xs whitespace-nowrap flex flex-col items-center min-w-[100px] border border-slate-600 gap-1">
                                <img src={visual} alt={item.name} className="w-10 h-10 rounded border border-slate-500" />
                                <span className="font-bold text-yellow-400">{item.name}</span>
                                <span className="text-slate-300">{item.cost} G</span>
                            </button>
                        );
                    })}
                </div>
          ...
          {gameState === 'BLACKSMITH' && (
             <div className="flex flex-col items-center h-full w-full p-2 overflow-y-auto">
                <h2 className="text-lg font-bold text-slate-300 mb-4 flex gap-2"><Hammer /> The Forge</h2>
                <div className="flex gap-4 justify-center items-center mb-6">
                    <button onClick={() => {
                        ...
                        <img src={getItemImage(player.equipment.weapon.name, player.equipment.weapon.rarity)} alt="Weapon" className="w-10 h-10 mb-2 rounded border border-slate-500" />
                    ...
                    <button onClick={() => {
                        ...
                        <img src={getItemImage(player.equipment.armor.name, player.equipment.armor.rarity)} alt="Armor" className="w-10 h-10 mb-2 rounded border border-slate-500" />
                ...
                {/* --- ENCHANT SECTION --- */}
                <div className="bg-slate-800 p-4 rounded-xl border border-purple-500/50 w-full max-w-md mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={() => handleEnchant('weapon')} className="bg-slate-700 hover:bg-slate-600 p-3 rounded border border-purple-900 group">
                        ...
                        <button onClick={() => handleEnchant('armor')} className="bg-slate-700 hover:bg-slate-600 p-3 rounded border border-purple-900 group">
                ...
                <button onClick={() => setGameState('IDLE')} className="bg-slate-600 hover:bg-slate-500 px-6 py-2 rounded h-fit text-sm">Leave</button>
             </div>
           )}
               ...
               {location.id === 'town' ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      <button onClick={() => setGameState('SHOP')} className="bg-slate-800 hover:bg-slate-700 p-3 rounded border border-slate-700 flex flex-col items-center group">
                          <img src="https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/rijfy4ijj5ypknyrcqj9wu69gij8" alt="market" className='w-16 h-16'/>
